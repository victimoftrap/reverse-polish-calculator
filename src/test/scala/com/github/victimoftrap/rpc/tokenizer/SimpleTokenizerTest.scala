...
class SimpleTokenizerTest extends AnyFlatSpec with Matchers {
  val tokenizer = new SimpleTokenizer

  "A SimpleTokenizer" should "return list with single token if input string without spaces" in {
    val expectedTokens = List(Token(TokenType.OPERATOR, "2+3"))
    val actualTokens = tokenizer.tokens("2+3")

    actualTokens.length should be(expectedTokens.length)
    actualTokens should be(expectedTokens)
  }

  "A SimpleTokenizer" should "return empty token list if empty string passed" in {
